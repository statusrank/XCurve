...
def get_mnist_datasets(train_transform, test_transform, train_classes=range(6),
    ...
    train_dataset_whole = CustomMNIST(root=mnist_root, transform=train_transform, train=True, download=True)
    ...
    test_dataset_known = CustomMNIST(root=mnist_root, transform=test_transform, train=False, download=True)
    test_dataset_known = subsample_classes(test_dataset_known, include_classes=train_classes)

    # Get testset for unknown classes
    test_dataset_unknown = CustomMNIST(root=mnist_root, transform=test_transform, train=False, download=True)
